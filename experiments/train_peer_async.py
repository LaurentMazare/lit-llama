...
def main(logs_dir: str = "logs", peer_id: int = -1, agg_interval: int = 100, batch_size: int = 25) -> None:
    ...
    first_checkpoint = f"checkpoint_agg_iter{agg_iter_num:04d}.ckpt"
    first_checkpoint_path = os.path.join(out_dir, first_checkpoint)
    print(f"Expecting {first_checkpoint}")
    while True:
        if os.path.exists(first_checkpoint_path):
            break
        time.sleep(1.0)

    with torch.no_grad():
        model.load_state_dict(torch.load(first_checkpoint_path))

    tmp_checkpoint = f"_checkpoint_tmp{peer_id:02d}.ckpt"
    torch.save(model.state_dict(), os.path.join(out_dir, tmp_checkpoint))

    loaded_checkpoints = [first_checkpoint]

    while True:
        iter_num = agg_iter_num * agg_interval

        train_peer(fabric, model, optimizer, peer_id, iter_num, agg_interval, loaded_checkpoints, batch_size, train_data)
...
def train_peer(
    ...
    while True:
        ...
        if not is_accumulating:
            ...
            agg_checkpoints = [el for el in os.listdir(out_dir) if el.startswith("checkpoint_agg_iter")]
            agg_checkpoints.sort()
            latest_checkpoint = agg_checkpoints[-1]

            if latest_checkpoint not in loaded_checkpoints:
                latest_checkpoint_path = os.path.join(out_dir, latest_checkpoint)
                ...
                with torch.no_grad():
                    base_state_dict = torch.load(latest_checkpoint_path, map_location="cpu")
                    for k in curr_state_dict:
                        curr_state_dict[k] -= base_state_dict[k]
                    del base_state_dict
                    gc.collect()
                    
                    last_state_dict = torch.load(latest_checkpoint_path, map_location="cpu")
                    for k in curr_state_dict:
                        curr_state_dict[k] += last_state_dict[k]
                    del last_state_dict
                    gc.collect()

                loaded_checkpoints.append(latest_checkpoint)
